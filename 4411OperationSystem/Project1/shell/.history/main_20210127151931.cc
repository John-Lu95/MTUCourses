...
void run_PipeElement(SSVec &InsPipes,int PipeElementIndex)
{
    ...
    for(itr=(*InsPip_Itr1).begin();itr!=(*InsPip_Itr1).end();itr++)
    {
        ...
        std::string inFileName=NULL, outFileName=NULL;
        std::string tmpStr0=Commands[TokenNum-1];
        if(tmpStr0==">" || tmpStr0=="<")
        {
...
void run_noPipe(SSVec &InsPipes)
{
    run_PipeElement(InsPipes,0);
    return;
}
...
void run_pipes(SSVec &InsPipes,int PipeElementNum,int PipeElementIndex)
{
    ...
    {
        run_PipeElement(InsPipes,PipeElementIndex);
        return;
    }
...
void parse_and_run_command(const std::string &command) {
    ...
    {
        if(std::count(command.begin(),command.end(),'|')!=(PipeComponents.size()-1))
        {
            std::cerr<<"invalid command."<<std::endl;
            return;
        }
    ...
    if(PENum==1)
    {
        run_noPipe(InstructionS_PipeLine);
    }
    else
    {
        run_pipes(InstructionS_PipeLine,PENum,0);
    }
